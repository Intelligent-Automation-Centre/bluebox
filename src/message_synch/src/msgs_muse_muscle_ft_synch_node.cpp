...
#include "muse_signal/muse_signal_msg.h"
#include "ros/ros.h"
#include <sstream>
#include <iostream>
...
public:
	Node():
		...
		muse_msgs_sub_(nh_,"/muse_signal",100),
		...
		muse_synch_signal_pub(nh_.advertise<muse_signal::muse_signal_msg>("/muse_signal_synch",100)),
        sync(MySyncPolicy(10),  ft_msgs_sub_,muscle_eg_sub_,muse_msgs_sub_){
             sync.registerCallback(boost::bind(&Node::callback, this,_1, _2,_3));
        }


void callback(const WrenchStampedConstPtr &ft_msgs,const muscle_eg_signalConstPtr &muscle_eg_s, const muse_signal::muse_signal_msgConstPtr& muse_sig)
       {
	   	  ROS_INFO("In callback");
	   	  muscle_eg_signal muscle_eg_sync1=*muscle_eg_s;
          WrenchStamped ft_msgs_sync=*ft_msgs;
          muse_signal::muse_signal_msg sync_muse=*muse_sig;
          muscle_egSynch_pub.publish(muscle_eg_sync1);
          ft_msgs_pub.publish(ft_msgs_sync);
          muse_synch_signal_pub.publish(sync_muse);
        }
private:
   ros::NodeHandle nh_;
   ...
   typedef message_filters::Subscriber<muse_signal::muse_signal_msg> muse_msgs_sub;
   muse_msgs_sub muse_msgs_sub_;
   typedef message_filters::Subscriber<WrenchStamped> ft_msg_sub;
   ft_msg_sub ft_msgs_sub_;
   typedef message_filters::sync_policies::ApproximateTime<WrenchStamped,muscle_eg_signal,muse_signal::muse_signal_msg> MySyncPolicy;
   message_filters::Synchronizer< MySyncPolicy > sync;
   ros::Publisher ft_msgs_pub;
   ros::Publisher muscle_egSynch_pub;
   ros::Publisher muse_synch_signal_pub;
};
